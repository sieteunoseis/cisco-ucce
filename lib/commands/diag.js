...
export default function (program) {
  ...
  const simpleCommands = [
    ...
    ["log-files", "List log files", "ListLogFiles", diagApi.listLogFiles],
    [
      "app-servers",
    ...
    .action(async (file, options) => {
      try {
        const results = await runOnHosts(
          ...
          "diag download-trace",
          "DownloadTraceFile",
          (client) => diagApi.downloadTraceFile(client, file),
          program.opts(),
        );
        await output(results, program.opts().format);
      } catch (err) {
    ...
    .action(async (file, options) => {
      try {
        const results = await runOnHosts(
          ...
          "diag download-log",
          "DownloadLogFile",
          (client) => diagApi.downloadLogFile(client, file),
          program.opts(),
        );
        await output(results, program.opts().format);
      } catch (err) {
